...
static int meminfo_ioctl(struct dfs_fd *file, int cmd, void *args) {
  switch (cmd) {
  case MEMINFO_DEV_CMD_READ_HEAP:
...
#define LIST_FIND_OBJ_NR 8
    ...
    if(sizeof(struct meminfo_t) != lwp_put_to_user(args, &meminfo, sizeof(struct meminfo_t))) {
        rt_kprintf("put_to_user failed 0x%08X\n", cmd);
    }
    break;
  case MEMINFO_DEV_CMD_READ_PAGE: {
    struct meminfo_t meminfo;
    size_t total, free;
